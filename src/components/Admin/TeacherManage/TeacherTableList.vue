<template>
    ...
    <div class="d-inline-flex col-12">
        <h5 class="float-left col-7" style="padding-top: 1rem;">Danh Sách giáo viên</h5>
        <div class="d-flex justify-content-end float-right col-5" style=";padding:0.5rem; margin-bottom: 10px;">
            <input type="text" class=" form-control" placeholder="Search" v-model = "this.searchContent">
            <div class="input-group-append">
                <button class="btn btn-success" @onclick = "searchTeacher()" type="submit">Search</button>
            </div>
        </div>
    </div>
    <!-- table -->
    ...
    <!-- pagging -->
    <PagingVue v-if = "this.isShowPaging" :totalPage = "this.totalpage" :currentPage = "this.currentPage" v-on:onClickToPage = "onMoveToPage"/>
</template>
...
export default {
    ...
    methods: {
        ...
        onMoveToPage(pageIndex) {
            ...
            router.push({ path: `list-teacher`, query : { page: pageIndex } } )
        },
        ...
        getListTeacher() {
            ...
            FetchApiWithCustomHeader(
                AccountAPI.GetListAccountByRoleName + `/TEACHER/${this.currentPage}/${this.searchContent}`,
                Method.GET,
                {
                ...
                (response) => {
                    if (response.result === "OK") {
                        this.updateTeacherList(response.data.resultList)
